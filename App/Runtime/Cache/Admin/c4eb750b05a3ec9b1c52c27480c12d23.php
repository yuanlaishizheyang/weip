      ...
      <form enctype="multipart/form-data" action="/Admin/Goods/add" method="post" name="theForm" >
            ...
            <td>
                <select name="cat_id"  >
                    ...
                    <?php foreach ($cats as $row):?>
                        <option value="<?php echo $row['id']?>" ><?php echo str_repeat('--',$row['level']).$row['cat_name']?></option>
                    <?php endforeach?>
                </select>
             </td>
          </tr>
        ...
        <table width="90%" id="mix-table" style="display:none" align="center">
          ...
          <table width="90%" id="attr-table" style="display:none">
              <tr>
                  <td><textarea rows="5" cols="30" name="goods_desc"></textarea></td>
              </tr>
          </table>
          <!--商品相册-->
          <table width="90%" id="album-table" style="display:none">
              <tr>
                  <td>
                          <p><a class="album" href="javascript:">[+]</a>上传文件<input type="file" name="album[]"></p>
                  </td>
              </tr>
          </table>
...
<script>
    $(function () {
        $('.album').live('click',function () {
            if ($(this).text()=='[+]') {
                var op = $(this).parent().clone();
                op.children('a').text('[-]');
                $(this).parent().parent().append(op);
            } else {
                $(this).parent().remove();
            }
        });
